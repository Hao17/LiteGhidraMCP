...
def ghidra_exec(
    ...
    client — so write operations share the client's existing checkout
    and never spawn a subprocess.

    Scripts have access to all Ghidra Flat API functions:
    ...
    Args:
        code: Python source with Flat API access.
        readonly: If True (default), refuses any write APIs. Set False to
                  allow modifications; the response includes `_saved` on
                  success.
        timeout: Max execution time in seconds (default: 120).

    Returns:
        result: Value of the 'result' variable (or null)
    ...
    Examples:
        ...
        # Batch rename with write access (use ghidra_edit's batch `actions`
        # instead when you just need rename/comment/datatype operations)
        code: "...", readonly: false
    """
    body = {
        "code": code,
