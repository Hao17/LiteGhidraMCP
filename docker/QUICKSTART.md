...
- Each client auto-generates an ephemeral UUID identity (or use `--user <name>` for a stable one)
...
└── 12.0.3/                          # Version directory
    ...
    └── ssh/clients/
        ├── bridgectl/              # gmcp CLI automation identity (admin +a everywhere)
        │   ├── ssh_key
        │   └── ssh_key.pub
        ├── u-a1b2c3d4e5f6/         # Ephemeral client identity (UUID, auto-generated)
        └── alice/                  # Named client identity (gmcp client start --user alice)
```

**Repos directory also holds:**
- `repos/.bridge-acl.conf` — explicit per-user grants (managed by `gmcp server repo grant/revoke`)
...
| User | Auth | Purpose |
|------|------|---------|
| Admin (you set) | Password | GUI access, auto-granted +a on all repos |
| `root` | Password (random per restart) | Fallback GUI access |
| `bridgectl` | SSH key (auto-generated by `gmcp server up`) | gmcp CLI automation (creates repos, etc.); +a on all |
| `u-<hex>` / `<name>` | SSH key (auto-generated by `gmcp client start`) | Per-client identity; only +w on explicitly-granted repos |
