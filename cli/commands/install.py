...
def skill(ctx):
    ...
    click.echo()
    click.echo("  Install skill (-d = target project directory):")
    click.echo("    gmcp install -d . skill claude-code   # → .claude/commands/ (symlinks)")
    click.echo("    gmcp install -d . skill codex         # → AGENTS.md")
    click.echo("    gmcp install -d . skill cursor        # → .cursor/rules/ghidra-mcp.md")
    click.echo("    gmcp install -d . skill copilot       # → .github/copilot-instructions.md")
    click.echo()
