...
# 创建仓库（admin-owned 模型：Client 无权自行创建仓库）
...
- **仅首次启动**：提示注册管理员用户（用户名 + 密码），用于 GUI 访问；管理员对所有仓库自动获得 `+a` 权限。
- `gmcp server repo create` 通过 `bridgectl` SSH 身份创建仓库（`bridgectl` 在首次 `server up` 时自动生成）
- 每次 `gmcp client start` 生成一个临时身份（`u-<hex>`）及其 SSH 密钥对，并在选定仓库上写入 `+w` 授权；如需固定身份，使用 `--user <name>`。
...
gmcp server add-user <name>              # 添加新的 password 管理员（交互式设置密码）
gmcp server repos                        # 列出仓库及其 ACL 授权
gmcp server repo create <r>              # 创建仓库（admin-owned）
gmcp server repo grant <user> <repo> +w  # 给非管理员 SSH 身份授予写权限
gmcp client logs 1                       # 查看 Client 1 日志
gmcp client stop 1                       # 停止 Client 1（同时清理其临时身份）
