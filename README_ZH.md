...
**推荐：Docker 部署（一条命令）** ⭐
- → [Docker Server-Client 模式](#选项-2separated-server-client-模式--推荐-aigui-协作)：AI + GUI 协作，一条命令启动
- 每个客户端独立用户隔离（bridge-1, bridge-2, ...），自动生成 SSH 密钥
- 适合生产环境、团队协作、多 AI Agent 场景

**替代方案：GUI 模式**
- → [在 Ghidra GUI 中运行 MCP](#快速开始gui-模式)：直接在 Ghidra 中运行脚本
- 无需 Docker，最适合学习和单人分析工作
...
| 特性 | 本地 GUI | Docker + 本地项目 | Docker + Separated Server ⭐ | Docker + 外部 Server |
|------|---------|------------------|------------------------------|----------------------|
| **使用场景** | 日常分析 | 仅自动化 | AI+GUI 协作 | 生产环境/团队 |
| **部署方式** | Ghidra GUI | Docker 容器 | 分离式容器 | Docker 容器 |
| **Ghidra 来源** | 本地安装 | 容器化 | 容器化 | 容器化 |
| **项目来源** | GUI 打开 | 卷挂载 (.gpr) | 独立 Ghidra Server | 外部 Ghidra Server |
| **AI + GUI 同时使用** | 不适用 | **否 - GUI 被锁定** | **是 - 完全协作** | **是 - 完全协作** |
| **部署复杂度** | 低 | 低 | **低 - 一条命令** | 中等 |
| **用户隔离** | 否 | 否 | **自动（bridge-1, bridge-2, ...）** | 手动配置 |
| **持久化** | 会话级 | 卷挂载 | 宿主机目录 | Server 存储 |
| **并发访问** | 单用户 | 容器锁定 | 多用户安全 | 多用户安全 |
| **版本控制** | 否 | 否 | 内置 | 内置 |

> **部署模式概要**：
> - **本地项目模式**：Docker 锁定项目 - **GUI 无法同时打开**
> - **Separated Server-Client 模式** ⭐：独立 Server + 可扩展 Client - **AI + GUI 可协同工作**，每客户端独立用户
> - **外部 Server 模式**：连接已有 Ghidra Server - 适合现有基础设施的生产环境
...
2. **Separated Server-Client** ⭐ **推荐** (`PROJECT_MODE=auto-server`)：独立 Server + 可扩展 Client
   - 适用于：AI + 人工协作，多 AI Agent，生产部署
   - 优势：一条命令部署，每客户端独立用户，AI 和 GUI 可协同工作
   - 启动：`make up-separated`

3. **外部 Ghidra Server** (`PROJECT_MODE=server`)：连接已有 Ghidra Server
   - 适用于：已有基础设施的生产环境
   - 优势：AI 和 GUI 可协同工作，企业级持久化
...
### 选项 2：Separated Server-Client 模式 ⭐ 推荐 AI+GUI 协作

**AI（Docker）+ GUI（人工）协同工作，一条命令部署！**

**配置：** `make up-separated`（一条命令）或 `make server-up && make client-up`（分步控制）

**特性：**

- ✅ **每客户端独立用户** - 每个客户端自动注册为 `bridge-1`、`bridge-2`、...
- ✅ **自动 SSH 密钥** - 每个客户端自动生成 PEM 格式 SSH 密钥对
- ✅ **AI + GUI 协作** - AI 客户端 + Ghidra GUI 可同时在同一项目上工作
- ✅ **多客户端支持** - 可扩展到多个 AI Agent，使用不同端口
- ✅ **持久化存储** - 数据在容器重启后保留
- ✅ **一条命令部署** - `make up-separated` 启动所有服务
...
# 一条命令启动 Server + Client
make up-separated

# 或分步控制
make server-up   # 启动 Server
make client-up   # 启动第一个客户端 (8803/8804) → 自动注册为 bridge-1
make client2-up  # 启动第二个客户端 (8813/8814) → 自动注册为 bridge-2
...
3. User: `root`
4. **取消勾选** "Use PKI authentication"
5. Password:（从 `make server-logs` 中获取，查找 `root (password): ...`）
6. Repository: `mcp-projects`

**创建的资源：**

- **Server**: Ghidra Server 端口 `13100`
- **Client**: HTTP API `http://localhost:8803`，MCP SSE `http://localhost:8804/sse`
- **用户**: `root`（密码，供 GUI 使用），`bridge-1`、`bridge-2`...（SSH 密钥，每客户端自动注册）
- **SSH 密钥**: `~/ghidra-data/<version>/ssh/clients/bridge-<N>/ssh_key`（每客户端自动生成）
- **仓库**: `mcp-projects`（自动创建）

**详细指南**: 参见 [docker/QUICKSTART.md](docker/QUICKSTART.md#separated-server-client-mode-recommended-for-aigui-collaboration)
...
- **持久化存储**（数据在容器删除后仍保留）
- **版本控制**（内置冲突解决）
- **多用户安全**（每个 AI 客户端作为独立用户）

**额外优势：**
- **权限隔离**：追踪哪些修改来自哪个 AI 客户端或人工
...
#### 高级：手动部署 Ghidra Server

如需自定义 Server 配置或连接到现有 Server：
