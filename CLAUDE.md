...
- **`scripts/`**: 独立脚本目录：
  ...
  - **`exec_runner.py`** + **`exec_runner_template.java`**: `/api/v1/exec` 的执行运行时（Python in-process / Java via headless `analyzeHeadless`），自动序列化 `result` 与捕获 stdout
...
- `POST /api/v1/exec` - 任意脚本执行（POST-only，不走 `@route` 装饰器，由 `do_POST` 直接处理）
  - 请求体: `{"code": "<source>", "language": "python"|"java", "readonly": true, "timeout": 120, "noanalysis": true}`
  - `language=python`: in-process 通过 `scripts/exec_runner.py` 执行，注入 Ghidra Flat API 全局变量（`currentProgram`、`toAddr`、`getFunctionAt` 等）。用户脚本里的 `result` 变量被 JSON 序列化返回，print 输出抓到 `stdout`
  - `language=java`: 通过 `analyzeHeadless` + `exec_runner_template.java` 在子进程执行（仅 Headless 模式可用），`noanalysis=true` 跳过自动分析
  - `readonly=false`: 由 `do_POST` 包裹 `ensure_checkout` + `auto_save`，与 `/api/v1/edit` 相同的 checkout/commit 生命周期
  - MCP proxy 暴露此端点供 `ghidra_exec` 工具调用（`mcp_sse_proxy.py:447`、`mcp_stdio.py:260`）
